...
def export_buttons():
    ...
    st.download_button("Download als CSV-Datei", st.session_state["csv_export"],
                       file_name=f"stocks_{result.year}.csv",
                       mime="text/csv")
    st.download_button("Download als Excel-Datei", st.session_state["excel_export"],
                       file_name=f"stocks_{result.year}.xlsx",
                       mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")


def display_stocks(result: Result, df_all: pd.DataFrame):
...
def prepare_exports(result: Result):
    ...
    st.session_state["excel_export"] = result.to_excel(f"Aktiengeschäfte {result.year}", columns,
                                                       ["quantity", "amount", "profit"])


report = ensure_report_is_available()
selected_year = ensure_selected_year()
report_result = report.get_stocks(selected_year)
st.session_state["report_result"] = report_result
prepare_exports(report_result)
display_stocks(report_result, report.get_all_stocks(selected_year))
